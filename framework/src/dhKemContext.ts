...
import { KdfContext, KdfFactory } from './kdfContext';
...
export abstract class DhKemContext implements KemContext {
    ...
    public async encap(pkR: XCryptoKey): Promise<{ sharedSecret: Uint8Array; enc: Uint8Array }> {
    ...
    public async authEncap(pkR: XCryptoKey, skS: XCryptoKey): Promise<{ sharedSecret: Uint8Array; enc: Uint8Array }> {
        const { privateKey: skE, publicKey: pkE } = await this.generateKeyPair();
